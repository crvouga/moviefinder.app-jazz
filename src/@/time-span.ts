...
const humanFriendlyString = (timeSpan: TimeSpan): string => {
	...
	if (days >= 365) {
		...
		return years >= 2
			? TRANSLATIONS.year.plural(years)
			: TRANSLATIONS.year.singular;
	}

	if (days >= 30) {
		const months = days / 30;
		return months >= 2
			? TRANSLATIONS.month.plural(months)
			: TRANSLATIONS.month.singular;
	}

	if (days >= 1) {
		return days >= 2
			? TRANSLATIONS.day.plural(days)
			: TRANSLATIONS.day.singular;
	}

	const hours = seconds / 3600;
	if (hours >= 1) {
		return hours >= 2
			? TRANSLATIONS.hour.plural(hours)
			: TRANSLATIONS.hour.singular;
	}

	const minutes = seconds / 60;
	if (minutes >= 1) {
		return minutes >= 2
			? TRANSLATIONS.minute.plural(minutes)
			: TRANSLATIONS.minute.singular;
	}
