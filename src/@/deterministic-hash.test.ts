...
describe("hashObject", () => {
  ...
  it("should hash arrays with same contents to same hash", () => {
    ...
    expect(toDeterministicHash(["a", "b"])).toBe(
      toDeterministicHash(["a", "b"])
    );
    expect(toDeterministicHash([null, true])).toBe(
      toDeterministicHash([null, true])
    );
  });

  it("should hash arrays with different contents to different hashes", () => {
    expect(toDeterministicHash([1, 2, 3])).not.toBe(
      toDeterministicHash([3, 2, 1])
    );
    expect(toDeterministicHash(["a", "b"])).not.toBe(
      toDeterministicHash(["b", "a"])
    );
  });
