...
const useCssModeSlidesOffsets = (
	...
	useLayoutEffect(() => {
		...
		if (props.slidesOffsetAfter) {
			slides[slides.length - 1].style.paddingRight =
				`${props.slidesOffsetAfter}px`;
		}
	}, [props]);
};
...
const useSwiperSlideChange = (
	...
	useEffect(() => {
		...
		const onSlideChange = (event: CustomEvent) => {
			...
			const activeIndex =
				typeof maybeActiveIndex === "number" ? maybeActiveIndex : 0;
