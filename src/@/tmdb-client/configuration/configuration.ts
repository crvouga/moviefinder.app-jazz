...
const toPosterImageSet = (
	...
	const lowestToHighestRes = posterSizes.map(
		(size) => `${baseUrl}${size}${posterPath}`,
	);
...
const toBackdropImageSet = (
	...
	const baseUrl = configuration.images?.secure_base_url ?? "";
	const lowestToHighestRes = backdropSizes.map(
		(size) => `${baseUrl}${size}${backdropPath}`,
	);
	const imageSet = ImageSet.init({ lowestToHighestRes });
	return imageSet;
};

const toProfileImageSet = (
	configuration: TmdbConfiguration,
	...
	const baseUrl = configuration.images?.secure_base_url ?? "";
	const lowestToHighestRes = profileSizes.map(
		(size) => `${baseUrl}${size}${profilePath}`,
	);
	const imageSet = ImageSet.init({ lowestToHighestRes });
	return imageSet;
};
