...
import type { TmdbClientConfig } from "../@/tmdb-client-config";
...
export type TmdbMovieCreditsQueryParams = z.infer<
	typeof TmdbMovieCreditsQueryParams
>;
...
export type TmdbMovieCreditsOkResponse = z.infer<
	typeof TmdbMovieCreditsOkResponse
>;
...
export const MovieCreditsClient = (config: TmdbClientConfig) => {
	return {
		get: HttpClientGet({
			config,
			endpoint: ({ movie_id }: { movie_id: number }) =>
